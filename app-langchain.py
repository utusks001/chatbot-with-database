...
def load_document(file_path, file_type):
    if file_type == ".pdf":
        ...
        return load_pptx_without_nltk(file_path)
...
with tab2:
    ...
    with col1:
        uploaded_files = st.file_uploader(
            ...
            type=["pdf","docx","pptx","txt","jpg","jpeg","png","bmp","tiff","gif"],
    ...
    with col2:
        if st.button("♻️ Reset Vectorstore"):
            st.session_state.vectorstore = None
            st.session_state.uploaded_rag_files = []
            st.success("Vectorstore & uploaded files di-reset.")
    ...
    if st.session_state.uploaded_rag_files:
        with st.spinner("🔍 Memproses dokumen..."):
            ...
            if docs:
                ...
                st.success(f"✅ {len(docs)} dokumen terproses ke Vectorstore.")

        st.subheader("💬 Chatbot RAG")
        query = st.text_input("Tanyakan sesuatu terkait dokumen:")
        if query and st.session_state.vectorstore:
            results = st.session_state.vectorstore.similarity_search(query, k=5)
            st.write("**Hasil RAG:**")
            for idx, r in enumerate(results):
                st.write(f"{idx+1}. {r.page_content[:500]}...")
