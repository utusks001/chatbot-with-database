...
# app.py - Ultra-lite final RAG + Data Analysis Chatbot
...
if not GOOGLE_API_KEY:
    ...
    if GOOGLE_API_KEY:
        ...
        st.sidebar.success("✅ GOOGLE_API_KEY berhasil dimasukkan")
        save_choice = st.sidebar.radio("Simpan key ke mana?", ["Jangan simpan", ".env", "secrets.toml"])
        if st.sidebar.button("💾 Simpan API Key"):
            if save_choice == ".env":
                set_key(dotenv_path, "GOOGLE_API_KEY", GOOGLE_API_KEY)
                st.sidebar.success("✅ API Key disimpan ke .env")
                ...
                with open(secrets_path, "w") as f:
                    toml.dump(secrets, f)
                st.sidebar.success("✅ API Key disimpan ke .streamlit/secrets.toml")
else:
    st.sidebar.success("✅ GOOGLE_API_KEY berhasil dimuat")
...
def build_vectorstore(files):
    ...
    for file in files:
        ...
        try:
            ...
            st.warning(f"File {name} gagal diproses: {e}")
...
# ====== Fallback HuggingFace LLM ======
def get_fallback_llm():
    hf_pipe = pipeline("text2text-generation", model="google/flan-t5-small", tokenizer="google/flan-t5-small")
...
with tab1:
    ...
    if uploaded_file:
        ...
        if not selected_sheets: st.warning("Pilih minimal satu sheet"); st.stop()
